...
const Badge = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <span className={`inline-flex items-center px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-bold uppercase tracking-[0.2em] ${className}`}>
    {children}
  </span>
);
...
const Section = ({ title, children, className = "", id = "" }: { title?: string, children: React.ReactNode, className?: string, id?: string }) => (
  <section id={id} className={`py-12 md:py-20 ${className}`}>
    <div className="max-w-6xl mx-auto px-4">
      {title && (
        <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center md:text-left tracking-tight">
...
const BlogPostPage = () => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#030711]">
        <div className="text-center">
          ...
          <button
            ...
            className="text-primary flex items-center gap-2 mx-auto hover:underline"
          >
  ...
  lines.forEach((line) => {
    ...
    if (line.startsWith('## ')) {
      ...
      // Skip as it's part of image metadata
      ...
      <section className="pb-24 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          {sections.map((section, idx) => {
            ...
            // Logic to detect if a section is "Features" (short list items = cards)
            ...
            const avgLength = listItems.length > 0
              ...
              : 0;
            // Only render as feature cards if: 3+ list items AND average text is short (< 100 chars)
            const isFeatures = listItems.length >= 3 && avgLength < 100;
            // Logic to detect if a section is "Steps" (numbered items)
